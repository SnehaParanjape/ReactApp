...
class TabPage extends Component {
	...
	handleInsertQuery(event) {
		...
		var q="INSERT { graph_or_triple_patterns } WHERE {TABLE ('csv', 'global', ' "+this.state.delimiter+" ' ,"+this.state.option+", ' "+this.state.columnMappingString+" ')\n"+binds;
		*/
		/*
		var data='';
		for(var j = 0; j < this.state.firstLine.length - 1; j++) {
		...
		*/
		...
		for ( var i = 1; i < this.state.fileContents.length; i++) {
	      	//console.log(this.state.fileContents[i]);
	      	//console.log(this.state.fileContents[i].split(','));
	      	var temp = this.state.fileContents[i].split(',');
	      	data += "\n[ ] <"+this.state.firstLine[0]+"> <"+temp[0]+">";
	      	for( var j = 1; j < temp.length-1; j++) {
	      		//console.log(temp[j]);
	      		//console.log(this.state.firstLine[j]);
	      		data += "\n <"+this.state.firstLine[j]+"> "+temp[j]+";";
	      	}
	      	data += "\n <"+this.state.firstLine[temp.length-1]+"> "+temp[temp.length-1]+".";
	    }
		var q="INSERT DATA { [ GRAPH <"+this.state.fileName+"> { "+data+"\n } }";
		*/
		//console.log(this.state.uriPrefixArray);
		var data = '';
		//console.log(this.state.firstLine);
		for ( var i = 1; i < this.state.fileContents.length; i++) {
	      	var temp = this.state.fileContents[i].split(',');
	      	/*if(this.state.predicateArray[0] !== null) {
	      		data += "\n[ ] <"+this.state.predicateArray[0]+"> <"+temp[0]+">";
	      	}
	      	else {
	      		data += "\n[ ] <"+this.state.firstLine[0]+"> <"+temp[0]+">";
	      	}*/
	      	...
	      	////////CHECK IF STRING OR PREDICATE OR FLOAT
	      	...
	      	for( var j = 0; j < temp.length-1; j++) {
	      		if(this.state.predicateArray[j] !== null) {
	      			...
	      			//if(this.state.types[j] === "float") {
    				//	data += "\n <"+this.state.predicateArray[j]+"> \""+temp[j]+"\"^^xsd:float;";
    				//}
	      			...
	      			//else if(this.state.types[j] === "float") {
    				//	data += "\n <"+this.state.firstLine[j]+"> \""+temp[j]+"\"^^xsd:float;";
    				//}
		      		//data += "\n <"+this.state.firstLine[j]+"> "+temp[j]+";";
		      	}
	      		//data += "\n <"+this.state.firstLine[j]+"> "+temp[j]+";";
		...
		return (
			<Tabs>
	    		...
	    		<TabPanel>
					...
					<div id="fifth_div">
						<div id="generate_insert_query_div">
							<button type="button" id="insert_query_button" onClick={this.handleInsertQuery}>Generate Insert Query</button>
						</div>

						<div id="generate_table_query_div">
							<button type="button" id="table_query_button" onClick={this.handleTableQuery}>Generate Table Query</button>
						</div>
					</div>
